...
class VouchStore(
    ...
    private val vouchMapper = {
        ...
        ->
        VouchEntry(
            pubKey = voucher_public_key,
            vouchAmount = vouch_amount,
            vouchUntil = vouch_until,
            bondId = bond_id
        )
    }
     ...
     */
    fun setVouch(
        voucherKey: ByteArray,
        voucheeKey: ByteArray,
        amount: Double,
        until: Long? = null,
        bondId: String
    ) {
    ...
    fun getVouch(
        ...
        database.dbVouchQueries
            .getVouch(voucherKey, voucheeKey, vouchMapper)
            .executeAsOneOrNull()
    ...
    fun getVouchesByVoucher(voucherKey: ByteArray): List<VouchEntry> =
        database.dbVouchQueries
            .getVouchesByVoucher(voucherKey, vouchMapper)
            .executeAsList()
    ...
    fun getVouchesByVouchee(voucheeKey: ByteArray): List<VouchEntry> =
        database.dbVouchQueries
            .getVouchesByVouchee(voucheeKey, vouchMapper)
            .executeAsList()
    ...
    /** Construct a list of guarantors from who actually vouched the payment
     *
     */
    fun getGuarantorsForUser(
        ...
        val now = System.currentTimeMillis()
        val vouches = getVouchesByVouchee(userKey)
        return vouches
            .filter {
                val until = it.vouchUntil
                until == null || until > now
            }.map { vouch ->
                val trustScore = trustStore.getScore(vouch.pubKey) ?: 0L
                Guarantor(
                    publicKey = vouch.pubKey,
                    trust = trustScore.toInt(),
                    vouchAmount = vouch.vouchAmount,
                    vouchUntil = vouch.vouchUntil
                )
            }
    }
