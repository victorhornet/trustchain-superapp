...
//    }
    ...
    /**
     * Collects vouch data for offline NFC exchange
     */
    fun collectVouchData(): String {
        val vouches = myVouchStore.getOwnActiveVouches().take(10)
        return serializeVouchData(vouches)
    }

    /**
     * Processes vouch data received via NFC
     */
    fun processVouchData(vouchDataString: String, senderPubKey: ByteArray) {
        try {
            val vouchDataList = parseVouchData(vouchDataString)
            for (vouchData in vouchDataList) {
                val vouch = Vouch(
            ...
            Log.e("EuroTokenCommunity", "Error processing vouch data via NFC", e)
        }
    }
